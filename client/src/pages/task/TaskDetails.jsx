...
const UserStatusWork = ({ task }) => {
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-md">
      <h3 className="text-xl font-semibold mb-4 text-white">User Work Status</h3>
      
      {task?.team?.map((item, index) => {
        const progress = userStatus[item?.status] || 0;
          ...
          <div key={index} className="mb-6">
            <div className="flex items-center space-x-4">
              <div className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center">
                {item?.name?.slice(0, 2).toUpperCase()} {/* Display initials */}
              </div>
              <div>
                <p className="text-lg font-medium text-cyan-400">{item?.name}</p>
            ...
            <div className="flex justify-between mt-2">
              <p className="text-sm text-white">{item?.status || 100}</p>
            </div>
          </div>
        );
      })}
